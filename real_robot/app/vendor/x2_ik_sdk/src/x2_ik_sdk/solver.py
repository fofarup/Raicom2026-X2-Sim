...
class X2ArmIKSolver:
    ...
    def solve_pose(
        ...
        *,
        current_head_pos: Iterable[float] | None = None,
        q_seed: np.ndarray | None = None,
        position_eps: float = 0.003,
        rotation_eps: float = 0.06,
    ) -> IKResult:
        """Solve a full end-effector pose while moving only the selected arm.

        ``target_rotation`` is the end-effector rotation in the URDF root
        (pelvis/base_link) frame.  Position-only IK is unsafe for a claw near a
        table because its unconstrained wrist orientation can change sharply.
        """
        side = ArmSide(side)
        target = np.asarray(list(target_xyz), dtype=float)
        rotation = np.asarray(target_rotation, dtype=float)
        if target.shape != (3,) or rotation.shape != (3, 3):
            raise ValueError("target_xyz must be (3,) and target_rotation must be (3,3)")
        ...
        for iterations in range(1, self.config.max_iters + 1):
            ...
            pos_err = target - placement.translation
            rot_err = pin.log3(rotation @ placement.rotation.T)
            error_norm = float(np.linalg.norm(pos_err))
            if error_norm < position_eps and float(np.linalg.norm(rot_err)) < rotation_eps:
                success = True
                break
            ...
            velocity = self._damped_least_squares(jacobian, error, active_v_idxs)
            step = velocity * self.config.dt
            step_norm = float(np.linalg.norm(step))
            if step_norm > self.config.max_step_norm:
                step *= self.config.max_step_norm / step_norm
        ...
        active_arm = arm_pos[:7] if side == ArmSide.LEFT else arm_pos[7:]
        return IKResult(
            success=success, side=side, arm_pos=arm_pos, active_arm=active_arm,
            target_xyz=target.tolist(), final_xyz=final.tolist(),
            error_norm=error_norm, iterations=iterations, ee_frame=frame_name,
