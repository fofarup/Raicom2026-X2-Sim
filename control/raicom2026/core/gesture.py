...
POSES = {
    ...
    # 双手打叉：双臂前伸交叉成 X（肩pitch前送 + yaw内旋交叉 + 肘深屈）
    # X2 肩roll不能内收，靠 shoulder_yaw 转前臂交叉，elbow 深屈缩短力臂
    "双手打叉": [
        # 左臂：前伸 → yaw大幅内旋 → 肘深屈 → 手到右侧
        -0.30, 0.0,  1.80, -1.80, 0.0, 0.0, 0.0,
        # 右臂：前伸 → yaw大幅内旋 → 肘深屈 → 手到左侧
        -0.30, 0.0, -1.80, -1.80, 0.0, 0.0, 0.0,
    ],
}
